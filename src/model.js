...
class Model {
  ...
  static async getSource(animeId, episodeId) {
    ...
    if (Array.isArray(sources.m3u8) ? !sources.m3u8.length : !sources.m3u8) {
      ...
      console.log("🚀 ~ file: model.js ~ line 96 ~ Model ~ getSource ~ sourceKey", sourceKey)
      if (sourceKey === "vip") {
  ...
  static async getAllAnime(page) {
    const { data } = await axios.get(`${WEBSITE_URL}/anime`);
    return parseList(data);
  }

  static async getByPage(page) {
    const { data } = await axios.get(`${WEBSITE_URL}/anime/${page}`);
    return parseList(data);
  }
